import random
from tkinter import messagebox
from tkinter import *


#Password generator function
def generate_password():
 try:
   repeat = int(repeat_entry.get())
   length = int(length_entry.get())
 except:
   messagebox.showerror(message="Please key in the required inputs")
   return
 #if user want repitition of characters :-
 if repeat == 1:
   password = random.sample(character_string,length)
 else:
   password = random.choices(character_string,k=length)
#convert returned values (list) into string using .join:-
 password =''.join(password)
# declare a string var:-
 password_v = StringVar()
 password="Cretaed password: "+str(password)
 #password assign to string variales
 password_v.set(password)
 #create a entry box for input/output:=
 password_label = Entry(password_gen, bd=0, bg="gray85", textvariable=password_v, state="readonly")
 password_label.place(x=10, y=140, height=50, width=320)

 # string conataing(letters, symbols and numbers)
character_string="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!#$%&'()*+,-./:;<=>?@[\]^_`{|}~"

 # define UI(user interface):-
password_gen = Tk()
password_gen.geometry("350x200")
password_gen.title("Python Password Generator")

 # Title of the app:-
title_lable = Label(password_gen, text="Python Password Generator", font=('Ubuntu Mono',12))
title_lable.pack()
 # length
length_lable = Label(password_gen, text="Enter length of password: ")
length_lable.place(x=20,y=30)
length_entry = Entry(password_gen, width=3)
length_entry.place(x=190,y=30)
 # repetition -
repeat_lable = Label(password_gen, text="Repetition? 1: no repetition, 2: otherwise: ")
repeat_lable.place(x=20,y=60)
repeat_entry = Entry(password_gen, width=3)
repeat_entry.place(x=300,y=60)
 # password button:
password_button = Button(password_gen, text="Generate Password", command=generate_password)
password_button.place(x=100,y=100)
 # Exit and Close App :
password_gen.mainloop()


  
 